...
//For each object in the array, access the first name and last name properties and concatenate them
//Push the concatenated pair to the new array, fullName
...
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array
// and log the result

let numberDonation = runners.map((runner) => {
    return parseInt(runner.donation);
})

let ticketPriceTotal = numberDonation.reduce((total, donation) => {
    return total += donation;
});

// let ticketPriceTotal = runners.reduce((total, runner) => {
//     return total += parseInt(runner.donation);
// })

console.log(ticketPriceTotal);
// console.log(parseInt(runners[1].donation));
